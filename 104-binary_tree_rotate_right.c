...
{
	binary_tree_t *root, *new_root, *new_root_lc, *root_p;
	...
	if ((root->left == NULL) && (root->right == NULL))
	{
		...
		{
			if (root_p->parent != NULL && (root->parent->left == root))
			{
				if (root_p->parent != NULL && (root_pparent->left == root_p))
					root_p->parent->left = root;
				if (root_p->parent != NULL && (root_p->parent->right == root_p))
					root_p->parent->right = root;
				root->parent = root_p->parent;
				root->left = NULL, root_p->parent = root;
				root->right = root_p;
			}
		}
		else
